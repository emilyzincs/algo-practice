from typing import assert_never, Any, override
from boilerplate.util import validate_type
from boilerplate.interface import BpInterface
from util.enums import ParseType, member_from_string


# C++ implementation of the boilerplate interface
class CppBp(BpInterface):

  @override
  def get_start(self) -> str:
    return ""
  
  @override
  def get_imports(self, included_types: set[ParseType]) -> str:
    includes = []
    using = []
    if ParseType.STRING in included_types:
      includes.append("string")
      using.append("std::string")
    if (
      ParseType.ARRAY in included_types or 
      ParseType.LIST in included_types or
      ParseType.UNORDERED_LIST in included_types
    ):
      includes.append("vector")
      using.append("std::vector")
    ret = ""
    for inc in includes:
      ret += f"#include <{inc}>"
    if includes and using:
      ret += "\n"
    ret += f"using {', '.join(using)};"
    if ret:
      ret += "\n"
    return ret

  @override
  def get_class_declaration(self, class_name, one_indent) -> str:
    return (f"class {class_name}" + " {\n" + f"{' ' * (len(one_indent) // 2)}"
            + "public:\n")

  @override
  def get_method_declaration(self, require_method_name: str, parameter_names: list[str], 
                             parameter_types: list[str], 
                            return_type: str, one_indent: str) -> str:
    in_parentheses = None
    n = len(parameter_names)
    if n == 0:
      in_parentheses = ""
    else:
      in_parentheses = f"{parameter_types[0]} {parameter_names[0]}"
      for i in range(1, n):
        in_parentheses += f", {parameter_types[i]} {parameter_names[i]}"
    return (f"{one_indent}static {return_type} {require_method_name}({in_parentheses})" +
            " {\n" + (one_indent * 2) + f"\n{one_indent}" + "}\n")

  @override
  def parse_type_string(self, typ: dict[str, Any]) -> str:
    validate_type(typ)
    curr_type: ParseType = member_from_string(ParseType, typ["type"])
    match curr_type:
      case ParseType.INT | ParseType.LONG:
        return "int"
      case ParseType.LONG:
        return "long long"
      case ParseType.FLOAT:
        return "double"
      case ParseType.BOOLEAN:
        return "bool"
      case ParseType.STRING:
        return "string"
      case ParseType.ARRAY | ParseType.LIST | ParseType.UNORDERED_LIST:
        return f"vector<{self.parse_type_string(typ["items"])}>"
      case _:
        assert_never(curr_type)

  @override
  def get_end(self, one_indent: str) -> str:
    return "};\n"
